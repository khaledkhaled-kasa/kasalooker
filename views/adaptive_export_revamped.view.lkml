...
t as (WITH skinny_table AS (SELECT PropShrt, PropCode, Building, Metric,
            ...
            CASE WHEN LAST_DAY(PARSE_DATE('%Y %b %d', CONCAT(RIGHT(column_name,4),LEFT(column_name,3),"01")),MONTH) < '2021-11-30' THEN 'Audited Month' -- This is where to adjust audited month
            WHEN LAST_DAY(PARSE_DATE('%Y %b %d', CONCAT(RIGHT(column_name,4),LEFT(column_name,3),"01")),MONTH) = '2021-11-30' THEN 'Audited Month (Latest)' -- This is where to adjust audited month latest
